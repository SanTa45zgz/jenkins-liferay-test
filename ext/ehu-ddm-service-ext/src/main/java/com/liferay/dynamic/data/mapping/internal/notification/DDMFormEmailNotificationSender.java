...
public class DDMFormEmailNotificationSender {
	...
	private String _getEmailSubject(DDMFormInstance ddmFormInstance)
		...
		if (Validator.isBlank(subject)) {
			subject = LanguageUtil.format(getResourceBundleExt(locale), "webform-lang-email.subject.admin", ddmFormInstance.getName(locale), false);
		}
		return subject;
	...
	private void _populateParameters(
		...
		**/
		...
		// para poder enviar un correo único en varios idiomas
		List<Locale> locales = getLocaleExt(ddmFormInstance);
		
		// hemos generado LocalizedData personalizada
		List<LocalizedData> localizedDatas = new ArrayList<>();
		
		for (final Locale locale : locales) {
			final LocalizedData localizedData = new LocalizedData();	
			localizedData.setFormName(ddmFormInstance.getName(locale));
			localizedData.setPages(_getPages(ddmFormInstance, ddmFormInstanceRecord, locale));
			localizedData.setSiteName(_getSiteName(ddmFormInstance.getGroupId(), locale));
			localizedData.setUserName(_getUserName(ddmFormInstanceRecord, locale));
			// i18n
			localizedData.setRb(getResourceBundleExt(locale));
			localizedData.setLocale(locale);

			localizedDatas.add(localizedData);
		}
		
		template.put("localizedDatas", localizedDatas);
	...
	/** 
	 * Nos permite obtener las properties de los languages modificados en el ext.
	 * 
	 * @param locale
	 * @return
	 */
	protected ResourceBundle getResourceBundleExt(Locale locale) {
		return ResourceBundle.getBundle("content.ext.Language", locale, getClass().getClassLoader(), UTF8Control.INSTANCE);
	}
	
	/**
	 * Recupera una lista de Locales ordenados
	 * @author UPV/EHU
	 * @param ddmFormInstance
	 * @return
	 * @throws PortalException
	 */
	protected List<Locale> getLocaleExt(DDMFormInstance ddmFormInstance) throws PortalException {
		return getOrderedAvailableLocales(ddmFormInstance.getAvailableLanguageIds());
	}


	/**
	 * Recupera un array con los idiomas ordenados
	 * @author UPV/EHU
	 * @param languageIds
	 * @return
	 */
	private List<Locale> getOrderedAvailableLocales(String [] languageIds) {
		final String [] ORDERED_LANGUAGE_IDS = new String [] {"eu_ES", "es_ES", "en_GB", "fr_FR", "de_DE"};
		final Locale[] locales = LocaleUtil.fromLanguageIds(languageIds);
	    final List<Locale> nuevaLista = new ArrayList<>();
	    for (final String languageId : ORDERED_LANGUAGE_IDS) {
	    	final Locale locale = LocaleUtil.fromLanguageId(languageId);
	        if (ArrayUtil.contains(locales, locale)) {
	        	nuevaLista.add(locale);
	        }
	    }
