 ...
 * UPV/EHU
 *
 *	- Si la URL de vuelta atras desde la visualizacion de un contenido completo en el publicador es de la parte publica del sitio web guest se quita de la URL la cadena /web/guest  
 *  - Si la preferencia "Mostrar el titulo del contenido" no esta chequeada (asi siempre por customizacion de la configuración), al visualizar el contenido 
 *	se muestra el enlace de volver, acompañado del texto fijo "Atras" en vez del titulo del contenido, SOLO SI el parametro GET redirect tiene valor. 
 *	- Si el parametro GET REdirect NO tiene valor no se muestra el enlace de volver con lo que se pueden conseguir URLs unicas para mostrar contenidos completos  
 */
--%>
...
/* UPV/EHU */
final String guestPublic = "/web/guest";
...
if (Validator.isNull(redirect)) {
	...
	redirect = ParamUtil.getString( originalRequest, "redirect" );
	if( redirect.contains( guestPublic ) ) {
		redirect = redirect.replace( guestPublic, "" ); 
    }
...
/* UPV/EHU */
boolean historyBack = Validator.isNull( redirect );
/* UPV/EHU */
	...
	<c:if test="<%= (showBackURL && Validator.isNotNull(redirect)) || assetPublisherDisplayContext.isShowAssetTitle() || (!print && Validator.isNotNull(assetActions)) %>">
		<div class="align-items-center d-flex mb-2">
			<p class="component-title h4">
				<%-- UPV/EHU Comprobar que sólo aparece atrás en el publicador activo --%>
				<% 
				String instanceId = themeDisplay.getPortletDisplay().getId().replace("com_liferay_asset_publisher_web_portlet_AssetPublisherPortlet_INSTANCE_", StringPool.BLANK);																		         
				
				if( Validator.isNotNull( redirect ) &&
					( redirect.contains( instanceId ) || originalRequest.getRequestURI().contains( instanceId ) ) ) {
				%>
				<c:if test="<%= showBackURL && Validator.isNotNull(redirect) %>">
					<%
						/* UPV/EHU */						
						if( Validator.isNotNull( originalRequest.getQueryString() ) && !historyBack ) {
					%>
						<clay:link
				...
				<%
				/* UPV/EHU */				
				} //Comprobar que sólo aparece atrás en el publicador activo
				%>
			</p>
