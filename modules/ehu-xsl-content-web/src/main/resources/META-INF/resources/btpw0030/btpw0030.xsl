<?xml version="1.0"?>
	...
	<xsl:stylesheet 
		...
		<div>
			<xsl:attribute name="class">xsl-content pas</xsl:attribute>
				<xsl:choose>
					...
					<xsl:when test="pagina/criteriosFiltro">
						<h1>
							<xsl:value-of select="languageUtil:get($locale,'ehu.xsl-content.title.btpw0030')"/>
						</h1>
						<form>
							<xsl:attribute name="id">search_form_pas</xsl:attribute>								
