<?xml version="1.0"?>
	...
	<xsl:stylesheet 
		...
		<xsl:variable name="p_cod_idioma" select="ehu:app/ehu:parameters/ehu:parameter[@name='p_cod_idioma']"/>
		
		<xsl:variable name="localeStr"> 	
			<xsl:choose>
				<xsl:when test="$p_cod_idioma = 'eu' or $p_cod_idioma = 'EUS'">
					<xsl:value-of select="localeUtil:fromLanguageId('eu_ES')" />
				</xsl:when>
				<xsl:when test="$p_cod_idioma = 'es' or $p_cod_idioma = 'CAS'">
					<xsl:value-of select="localeUtil:fromLanguageId('es_ES')" />					
				</xsl:when>
				<xsl:when test="$p_cod_idioma = 'en' or $p_cod_idioma = 'ING'">
					<xsl:value-of select="localeUtil:fromLanguageId('en_GB')" />
				</xsl:when>
				<xsl:when test="$p_cod_idioma = 'fr'">
