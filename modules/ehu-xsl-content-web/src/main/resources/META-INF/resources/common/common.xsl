<?xml version="1.0"?>
	...
	<xsl:stylesheet 
		...
		<!-- Codigo de año -->
		<xsl:variable name="p_anyo_pop" select="ehu:app/ehu:parameters/ehu:parameter[@name='p_anyo_pop']"/>
		...
		<xsl:template name="link_document_graduate">
			...
			<xsl:param name="extension"/>
		  	...
		  	<a>
				...
				<xsl:attribute name="href">?p_redirect=descargaFichero&amp;p_cod_proceso=plew0040&amp;p_tipo=<xsl:value-of select="$document_type"/>&amp;p_cod_titulo=<xsl:value-of select="$document_cod_title"/>&amp;p_anyo_acad=<xsl:value-of select="$document_anyo"/></xsl:attribute>
		...
		<xsl:template name="link_guia_docente">
			...
			<xsl:param name="extension"/>
		  	...
		  	<a>		 
		  		...
		  		<!-- No metemos los campos p_cod_proceso y p_cod_master porque se meten después automáticamente -->	
		  		<xsl:attribute name="href">?p_redirect=descargaFichero&amp;p_cod_idioma=<xsl:value-of select="$document_cod_idioma"/>&amp;p_tipo=<xsl:value-of select="$document_type"/>&amp;p_anyo_pop=<xsl:value-of select="$document_anyo_pop"/>&amp;p_cod_materia=<xsl:value-of select="$document_cod_materia"/></xsl:attribute>
				<xsl:attribute name="target">_blank</xsl:attribute>
				<span>
			  	<xsl:choose>
			  		<xsl:when test="$extension != $void">
		...
		<xsl:template name="link_document_doctor">
			...
			<xsl:param name="extension"/>
		  	...
		  	<a>
				...
				<xsl:attribute name="href">?p_redirect=descargaFichero&amp;p_cod_proceso=plew0060&amp;p_tipo=<xsl:value-of select="$document_type"/></xsl:attribute>
				<xsl:attribute name="target">_blank</xsl:attribute>
				<span class="hide-accessible">
					<xsl:attribute name="class">hide-accessible</xsl:attribute>
					<xsl:value-of select="languageUtil:get($locale,'opens-new-window')"/>
