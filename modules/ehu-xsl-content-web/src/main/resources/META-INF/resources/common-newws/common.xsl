<?xml version="1.0"?>
	...
	<xsl:stylesheet 
		...
		<xsl:include href="http://localhost:8080/o/ehu-xsl-content-web/common-newws/common-lang.xsl"/>
		...
		<xsl:variable name="proceso" select="ehu:app/ehu:parameters/ehu:parameter[@name='proceso']"/>
		
		<!-- Codigo de información a mostrar -->
		<xsl:variable name="navegacion" select="ehu:app/ehu:parameters/ehu:parameter[@name='navegacion']"/>
		
		<!-- Entorno de ejecucion -->
		<xsl:variable name="host" select="propsUtil:get('ehu.host')"/>
		
		<!-- Origen de datos -->
		<xsl:variable name="xml_src_id"><xsl:value-of select="$proceso"/>.src</xsl:variable>
		<xsl:variable name="xml_src" select="propsUtil:get($xml_src_id)"/>
				
		<!-- Proceso que genera el XML -->
		<xsl:variable name="xml_proccess_id"><xsl:value-of select="$proceso"/>.xml</xsl:variable>
		...
		<xsl:template name="back_to">
			...
			<div>
				...
				<span> 
					...
					<a>
						<xsl:attribute name="href">?idioma=<xsl:value-of select="$idioma"/>&amp;proceso=<xsl:value-of select="$proceso"/></xsl:attribute>
						<span>
						...
						<li>
							<a>
								...
								<xsl:choose>
									...
									<xsl:otherwise>
										<!-- Pasamos las pestañas a mayúsculas -->
		...
		<xsl:template name="link_document_graduate">
			...
			<xsl:param name="extension"/>
		  	...
		  	<a>
				...
				<xsl:attribute name="href">?p_redirect=descargaFichero&amp;proceso=plew0040&amp;p_tipo=<xsl:value-of select="$document_type"/>&amp;p_cod_titulo=<xsl:value-of select="$document_cod_title"/>&amp;p_anyo_acad=<xsl:value-of select="$document_anyo"/></xsl:attribute>
		...
		<!-- Documento de Guía Docente -->
		<xsl:template name="link_guia_docente">
			<xsl:param name="document_cod_proceso"/>
			<xsl:param name="document_cod_idioma"/>
			...
			<xsl:param name="extension"/>
		  	...
		  	<a>		 
		  		<!-- Creamos el link en base a los parámetros recibidos --> 	
		  		<!-- No metemos los campos proceso y p_cod_master porque se meten después automáticamente -->	
		  		<xsl:attribute name="href">?p_redirect=descargaFichero&amp;idioma=<xsl:value-of select="$document_cod_idioma"/>&amp;p_tipo=<xsl:value-of select="$document_type"/>&amp;p_anyo_pop=<xsl:value-of select="$document_anyo_pop"/>&amp;p_cod_materia=<xsl:value-of select="$document_cod_materia"/></xsl:attribute>
				<xsl:attribute name="target">_blank</xsl:attribute>
				<span>
			  	<xsl:choose>
			  		<xsl:when test="$extension != $void">
		...
		<xsl:template name="link_document_master">
			...
			<xsl:param name="extension"/>
		  	...
		  	<a>
		  		<xsl:attribute name="href">?p_redirect=descargaFichero&amp;p_tipo=<xsl:value-of select="$document_type"/>&amp;p_anyo_pop=<xsl:value-of select="$document_anyo_pop"/>&amp;p_cod_materia=<xsl:value-of select="$document_cod_materia"/>&amp;p_fase=<xsl:value-of select="$document_fase"/>&amp;p_anyo_inf=<xsl:value-of select="$document_anyo_inf"/></xsl:attribute>
				<xsl:attribute name="target">_blank</xsl:attribute>
		  		<span>
			  	<xsl:choose>
			  		<xsl:when test="$extension != $void">
		...
		<xsl:template name="link_document_doctor">
			...
			<xsl:param name="extension"/>
		  	...
		  	<a>
				...
				<xsl:attribute name="href">?p_redirect=descargaFichero&amp;proceso=plew0060&amp;p_tipo=<xsl:value-of select="$document_type"/></xsl:attribute>
				<xsl:attribute name="target">_blank</xsl:attribute>
				<span class="hide-accessible">
					<xsl:attribute name="class">hide-accessible</xsl:attribute>
					<xsl:value-of select="languageUtil:get($locale,'opens-new-window')"/>
		...
		<xsl:template name="contact">
			...
			<xsl:param name="localizacion"/>
				<div>
					...
					<dl>
					<!-- Comprobamos que el nombre no llegue vacío para así no pintar "Nombre" -->
					<xsl:if test="$nombre != $void">										
		...
		<!-- Contacto Ikertu -->
		<xsl:template name="contact-ikertu">
			<xsl:param name="contact"/>
			<xsl:param name="nombre"/>
			<xsl:param name="localizacion"/>
				<div>
					...
					<xsl:for-each select="$contact">
						...
						<xsl:if test="ehu:web">
							<xsl:for-each select="$contact/ehu:web">
								<!-- Comprobamos si la url viene con http o no -->
								<xsl:choose>
									<xsl:when test="not(contains($contact/ehu:web, 'http'))">
										<xsl:call-template name="link_web">
											<xsl:with-param name="link" select="concat('http://',$contact/ehu:web)"/>								
										</xsl:call-template>
									</xsl:when>
									<xsl:otherwise>
										<xsl:call-template name="link_web">
											<xsl:with-param name="link" select="$contact/ehu:web"/>								
										</xsl:call-template>
									</xsl:otherwise>
								</xsl:choose>				
							</xsl:for-each>	
		...
		<!-- Contacto Máster -->
		<xsl:template name="contact-master">
			<xsl:param name="contact"/>
				<div>
					<xsl:attribute name="class">contact</xsl:attribute>
					<dl>
		...
		<xsl:template name="study-plan">
			...
			<div>
				...
				<xsl:call-template name="header_normal">
					...
					<xsl:with-param name="title">
						...
						<xsl:value-of select="$study-plan/ehu:curso[ehu:anyoImpart = $anyoAcad]/ehu:descAnyoImpart"/>
					</xsl:with-param>														
				</xsl:call-template>
				<xsl:for-each select="$study-plan/ehu:curso">
					<!-- Comprobamos que sean asignaturas del Año académico actual -->
					<xsl:if test="$anyoAcad = self::node()/ehu:anyoImpart">
			...
			<xsl:if test="$study-plan/ehu:curso[ehu:anyoImpart='20200']">
			<div>
				...
				<xsl:call-template name="header_normal">
					<xsl:with-param name="icon"><xsl:value-of select="$void"/></xsl:with-param>
					<xsl:with-param name="title"> 
						<xsl:value-of select="$study-plan/ehu:curso[ehu:anyoImpart='20200']/ehu:descAnyoImpart"/>
					</xsl:with-param>														
				</xsl:call-template>
				
				
				<xsl:for-each select="$study-plan/ehu:curso[ehu:anyoImpart='20200']">
			 		<h2>
					...
					<div>
						...
						<dl>
							...
							<dd><xsl:value-of select="ehu:descCaracter"/></dd>
							<dt><xsl:value-of select="languageUtil:get($locale,'ehu.teaching-language')"/></dt>
							<dd><xsl:value-of select="$study-plan/ehu:curso/ehu:idiomas/ehu:idioma/ehu:descIdiomaImpart"/></dd>					
						</dl>
						<xsl:if test="ehu:profesorado">
							<h3>
								<xsl:value-of select="languageUtil:get($locale,'ehu.teachers')"/>																		
			...
			<xsl:if test="$study-plan/ehu:curso[ehu:anyoImpart='20210']">
			<div>
				
				<xsl:attribute name="id">toggler3</xsl:attribute>
				
				<xsl:call-template name="header_normal">
					<xsl:with-param name="icon"><xsl:value-of select="$void"/></xsl:with-param>
					<xsl:with-param name="title"> 
						<xsl:value-of select="$study-plan/ehu:curso[ehu:anyoImpart='20210']/ehu:descAnyoImpart"/>
					</xsl:with-param>														
				</xsl:call-template>
				
				
				<xsl:for-each select="$study-plan/ehu:curso[ehu:anyoImpart='20210']">
			 		<h2>
						<xsl:attribute name="class">header3 toggler-header-collapsed</xsl:attribute>
						<xsl:value-of select="ehu:descCurso"/>
						<span>
							<xsl:attribute name="tabindex">0</xsl:attribute>
							<span>
					...
					<div>
						<xsl:attribute name="class">content3 toggler-content-collapsed</xsl:attribute>
						<dl>
							<dt><xsl:value-of select="languageUtil:get($locale,'ehu.number-of-credits')"/></dt>
							<dd><xsl:value-of select="ehu:numCreditos"/></dd>
							<dt><xsl:value-of select="languageUtil:get($locale,'ehu.study-type')"/></dt>
	...
	<xsl:template name="profesor-tutorias">
		...
		<xsl:if test="$tutorias">
			<xsl:call-template name="header_main">
				<xsl:with-param name="title" select="languageUtil:get($locale,'ehu.tutorias')"/>
			</xsl:call-template>			
			<table>
					...
					<xsl:for-each select="$docencias/ehu:docencia">
							<tr>
								...
								<td style="text-align:center">
									<xsl:value-of select="format-number(ehu:duracionDocente,'###.#')" />
								</td>
								<td style="text-align:center">
									<xsl:value-of select="format-number(ehu:duracionAlumno,'###.#')" />
								</td>
							</tr>
					</xsl:for-each>
					</tbody>
	...
	<xsl:template name="examenes-group">
		...
		<xsl:if test="$examenes">
			...
			<xsl:for-each select="$examenes/ehu:examen">
				...
				<xsl:variable name="xml_fmtFechas" select="'yyyy-MM-dd'"/>
				<xsl:value-of select="ehu:mes" /> : 
				<xsl:if test="$fechaHora != $void">
					<xsl:variable name="init_date_wz" select="substring-before($fechaHora,'Z')" />
					<xsl:variable name="yearInit" select="substring($init_date_wz,1,4)" />
					<xsl:variable name="monthInit" select="substring($init_date_wz,6,2)" />
					<xsl:variable name="dayInit" select="substring($init_date_wz,9,2)" />
					<xsl:variable name="hourInit" select="substring($init_date_wz,12,2)" />
					<xsl:variable name="minInit" select="substring($init_date_wz,15,2)" />
					<xsl:if test="languageUtil:getLanguageId($locale) != 'eu_ES'">
						<xsl:if test="$fechaHora != $void">
							<xsl:value-of select="$dayInit"/>-<xsl:value-of select="$monthInit"/>-<xsl:value-of select="$yearInit"/><xsl:value-of select="$white_space"/><xsl:value-of select="$hourInit"/>:<xsl:value-of select="$minInit"/>
						</xsl:if>	
					</xsl:if>
				
					<xsl:if test="languageUtil:getLanguageId($locale) = 'eu_ES'">
						<xsl:if test="$fechaHora != $void">
							<xsl:value-of select="$yearInit"/>-<xsl:value-of select="$monthInit"/>-<xsl:value-of select="$dayInit"/><xsl:value-of select="$white_space"/><xsl:value-of select="$hourInit"/>:<xsl:value-of select="$minInit"/>
						</xsl:if>	
	...
	<!-- Parsea el texto obtenido como parametro para montar parrafos teniendo en cuenta retornos de carro (cr) y fines de lineas (eol) -->
	<xsl:template name="transform_text">
  	...
  	<!--  Template para realizar el replace all de cadenas -->
