...
<html class="ltr" dir="ltr">
	<head>
		...
		<title>${languageUtil.get(rb, "upv-ehu.webform.send.confirmation.title.html")}</title>
		<style type="text/css">
			body {
				color: #54595f;
	...
	<body>
			<div id="container" style="border-bottom: 1px dotted #ccc;" lang="${locale.getLanguage()}">
				...
				<article>
					<h1>${languageUtil.get(rb, "upv-ehu.webform.send.confirmation.title")}: <a href="${viewFormURL}" style="font-size:0.9em;text-decoration:none;">${formName}</a></h1>
