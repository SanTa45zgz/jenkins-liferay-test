...
Nota:se usa para listar contenidos en ehu-theme, campusa y global-theme
-->
						...
						<#elseif ddmStructure.getName()?contains("Subvention")>
                            ...
                            <#if (ddmTemplateKey != 0)>
								...
								<#if !isOutStanding >
									...
									<#assign articleContent = journalArticleLocalService.getArticleContent(journalArticle, ddmTemplateKeyStr,"VIEW", locale, themeDisplay) >
									
									<#-- Se controla que exista el article -->
                                    <#if journalArticleLocalService.fetchDisplayArticle(journalArticle.getGroupId(),assetRenderer.getArticle().getArticleId())?? >    
