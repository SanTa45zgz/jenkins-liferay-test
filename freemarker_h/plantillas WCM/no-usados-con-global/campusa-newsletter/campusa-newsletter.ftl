...
<#macro viewArticles langPri = "" langSec = "" > 
	...
	<div lang="${langPri}">
									...
									<#if current_new.getData()?has_content>
										...
										<#if article?? >
											...
											<#list rootElement.elements() as dynamicElement >
												<#if dynamicElement?? && dynamicElement.attributeValue("name")??>
													<#local dynamicAttrValueName = dynamicElement.attributeValue("name") >
												<#else>
													<#local dynamicAttrValueName = "" >
												</#if>
												<#if dynamicAttrValueName == "ehuimages" >							   
													<#list dynamicElement.elements() as embeddedElement >
														<#if embeddedElement?? && embeddedElement.attributeValue("name")??>
															<#local embeddedAttrValueName = embeddedElement.attributeValue("name") >
														<#else>
															<#local embeddedAttrValueName = "" >
														</#if>
														<#if embeddedAttrValueName != "" >
															<#local embeddedElementContent = embeddedElement.element( "dynamic-content" ).getText() >
															<#if embeddedAttrValueName == "ehuhighlighttitle" && embeddedElementContent?has_content>
																<#local titulo = embeddedElementContent >
															<#elseif embeddedAttrValueName == "ehuhighlightsubtitle" && embeddedElementContent?has_content >
																<#local subtitulo = embeddedElementContent >
															<#elseif embeddedAttrValueName == "ehuhighlightimage" && embeddedElementContent?has_content>
																<#local imageContent = embeddedElementContent >
																<#local image = ( ( "\"" + imageContent?json_string + "\"" )?eval )?eval >
																...
																<#list embeddedElement.elements() as imageElement >
																	<#if imageElement?? && imageElement.attributeValue("name")??>
																		<#local imageAttrValueName = imageElement.attributeValue("name") >
																	<#else>
																		<#local imageAttrValueName = "" >
																	</#if>					
																	<#if imageAttrValueName != "" >
																		<#if imageAttrValueName == "ehuhighlightimagealttext" >
																			<#local altImage = imageElement.element( "dynamic-content" ).getText() >
																		</#if>
																	</#if>
																</#list>
															</#if>
														</#if> <#-- dynamicElement.elements()   -->
		...
		<#-- EVENTOS -->
		<#if ehu_event_list_articleId?? && ehu_event_list_articleId.getSiblings()?has_content && ehu_event_list_articleId.getSiblings()[0].getData()?has_content>
								...
								<#list ehu_event_list_articleId.getSiblings() as current_event>
									<#if current_event.getData()?has_content>
										...
										<#if article?? >
											...
											<#assign ehuimages = saxReaderUtil.createXPath("dynamic-element[@name='ehuimages']").selectSingleNode(rootElement) >
											<#local titulo = ehuimages.selectSingleNode("dynamic-element[@name='ehuhighlighttitle']").getStringValue()?trim >
											<#local subtitulo = ehuimages.selectSingleNode("dynamic-element[@name='ehuhighlightsubtitle']").getStringValue()?trim >

											<#attempt>
												<#assign imageJson = ehuimages.selectSingleNode("dynamic-element[@name='ehuhighlightimage']/dynamic-content").getStringValue()?trim?eval >
													
												<#local imageUrl = portalURL + "/documents/" + imageJson.groupId + "/" + imageJson.uuid >
												<#local altImage = ehuimages.selectSingleNode("dynamic-element[@name='ehuhighlightimage']/dynamic-element[@name='ehuhighlightimagealttext']").getStringValue()?trim >
											<#recover>
												<#local imageUrl = noImage >
												<#local altImage = txtNoImg >
											</#attempt>
											...
											<#attempt>
												<#assign ehugeneraldata = saxReaderUtil.createXPath("dynamic-element[@name='ehugeneraldata']").selectSingleNode(rootElement) >
												<#assign fecha = "">
												<#assign hora = "">
												<#assign minutos = "">
												<#if ehugeneraldata.selectSingleNode("dynamic-element[@name='ehustartdatehour']")??>
													<#assign fecha = ehugeneraldata.selectSingleNode("dynamic-element[@name='ehustartdatehour']").getStringValue()?trim?date("yyyy-MM-dd") >														
												</#if>
												<#if ehugeneraldata.selectSingleNode("dynamic-element[@name='ehustartdatehourhh']")??>
													<#assign hora = ehugeneraldata.selectSingleNode("dynamic-element[@name='ehustartdatehourhh']").getStringValue()?trim >
												</#if>
												<#if ehugeneraldata.selectSingleNode("dynamic-element[@name='ehustartdatehourmm']")??>
													<#assign minutos = ehugeneraldata.selectSingleNode("dynamic-element[@name='ehustartdatehourmm']").getStringValue()?trim >
												</#if>
												<#if hora?has_content>
